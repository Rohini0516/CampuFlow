    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#2D2526] tracking-tight flex items-center gap-2">
            <Calendar className="w-7 h-7 text-[#E27B88]" />
        ...
        {['ALL', 'TECHNICAL', 'CULTURAL', 'WORKSHOP', 'SPORTS', 'SEMINAR'].map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              filterCategory === cat
                ? 'bg-gradient-to-r from-[#E27B88] to-[#D9828B] text-white shadow-md'
                : 'bg-white text-[#6F6264] hover:bg-[#FFF5F1] border border-[#F0D9D5]'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredEvents.length === 0 ? (
          ...
          filteredEvents.map((ev) => (
            <div
              key={ev._id}
              className="bg-white rounded-3xl border border-[#F0D9D5] overflow-hidden shadow-sm hover:shadow-xl hover:border-[#EFA7B5] transition-all flex flex-col justify-between group"
            >
              <div className="p-6">
                <div className="flex items-center justify-between gap-2 mb-3">
                  <Badge variant={ev.category === 'TECHNICAL' ? 'peach' : 'default'} size="sm">
                    {ev.category}
                  </Badge>
                  <span className="text-[11px] font-bold text-[#A95763]">
                    Max: {ev.maxParticipants || 150} Seats
                  </span>
                </div>

                <h3 className="text-lg font-black text-[#2D2526] mb-2 group-hover:text-[#E27B88] transition-colors">
                  {ev.title}
                </h3>
                <p className="text-xs text-[#6F6264] line-clamp-3 mb-4 font-medium">{ev.description}</p>
              </div>

              <div className="px-6 py-4 bg-[#FFF5F1]/60 border-t border-[#F0D9D5] space-y-3">
                <div className="space-y-1.5 text-xs text-[#6F6264] font-medium">
      ...
      >
        <form onSubmit={handleCreateEvent} className="space-y-4 text-xs sm:text-sm">
          <div>
            <label className="block font-bold text-[#2D2526] mb-1">Event Title *</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-bold text-[#2D2526] mb-1">Venue *</label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-[#2D2526] mb-1">Start Date & Time *</label>
              <input
                type="datetime-local"
                required
            ...
            <div>
              <label className="block font-bold text-[#2D2526] mb-1">End Date & Time</label>
              <input
                type="datetime-local"
          ...
          <div>
            <label className="block font-bold text-[#2D2526] mb-1">Event Description</label>
            <textarea
              rows={3}
